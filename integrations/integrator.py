...
# One thing to note is you need to already have the rate created and linked to the property and at least 1 season.

def integrate(testing, moto_key, moto_secret, season_request, rates_request, pricelabs_api_key, pricelabs_id,
              accomodation_id, property_name):
    ...
    if not testing:
        ...
        rates_id = get_rates_id(existing_rates, accomodation_id)
        ...
        moto_data_to_add = compare_prices(motopress_data, pricelabls_data, rates_request, basic, property_name)
        response = create_rate(moto_data_to_add, rates_request, basic, accomodation_id, rates_id, property_name)
        ...
        existing_rates = get_rate_data(rates_request, basic)
        # print(f"existing rates {existing_rates}")
        # rates_id = existing_rates[0]['id']
        # print(f"rates_id {rates_id}")
        rates_id = get_rates_id(existing_rates, accomodation_id)
...
def compare_prices(motopress, pricelabs, rates_request,
                   basic, property_name):  # We pass in the request and auth because we will update from here.
...
def get_rate_data(url, basic):
    '''This is the base function to get rate data from motopress. It will send a request to the URL
    and return a list with the info inside.'''
...
def create_rate(season_info, url, basic, accomodation_id, rates_id, property_name):
    ...
    text = {
        "id": rates_id,
        "status": "active",
        "title": f"{property_name} Booking Rate",
...
def get_rates_id(existing_rates, accomodation_id):
    '''Function will loop through existing rates in motopress, find the one that has the matching accomotation id,
    and return the id for the rate.'''

    for i in existing_rates:
        if i["accommodation_type_id"] == accomodation_id:
            # print(f"get_rates_id will return: {i['id']}")
            return i["id"]
    print("If this prints, there was a problem and the rates dont have an accomodation id that matches whats in"
          "the database.")
#
#
